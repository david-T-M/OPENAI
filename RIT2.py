...
def incompatibilidad_semantica(palabra1,pos1,palabra2,pos2):
    ...
    if pos1=="VERB" and pos2=="VERB":
        ...
        bandera=0
        for a in wn.synsets(palabra1,pos=wn.NOUN): #NOUN
            contextos_a.append(a)
        contextos_b = []
    ...
    for b in contextos_b:
        for a in contextos_a:
            if type(a.shortest_path_distance(b))==type(1):
                if(a.shortest_path_distance(b)<=2):
                    print("encontró",a,b)
                    return 1
    if bandera==0:
        return 0
...
for i in range(len(textos)):
    ...
    sumas.append(ma.sum().sum())# cambie a minimo antes era suma
    if len(ma.columns)==0:
        max_info.append(0)
    else:
    ...
    if len(ma.columns)>0:
        mearts.append(m_earth.min().sum()/len(ma.columns))#suma de sumas lo cambie a max
        mutinf.append(m_mi.max().sum()/len(ma.columns))#cambie de suma a max
        distancias.append(m_distancia.max().sum()/len(ma.columns)) #cambie de maximas a sumas
    else:
        mearts.append(0)#suma de sumas lo cambie a max
        mutinf.append(0)#cambie de suma a max
        distancias.append(0) #cambie de maximas a sumas
    ...
    for t__1 in ma.index:
        for t__2 in ma.columns:
            print(t__1,t__2)
            incomp+=incompatibilidad_semantica(str(t__1).split("{")[0],str(t__1).split("}")[0].split(",")[1],str(t__2).split("{")[0],str(t__2).split("}")[0].split(",")[1])
...
temp1 =np.array([sumas,distancias,entropias,mutinf,mearts,max_info,list_antonimos,lista_related,similitud_faltantes,lista_incomp,lista_wmd,clases])
df_resultados = pd.DataFrame(temp1.T,columns=["suma","distancias","entropias","mutual_info","m_earth","max_info_p","antonimos","relaciones","sim_faltantes","incompatibilidad","wmd","CLASS"])
