...
def wasserstein_mutual_inf(texto_v,hipotesis_v,texto_t,texto_h):  
    ...
    for i in range(len(texto_t)):
        ...
        for j in range(len(texto_h)):
            ...
            lista_mu.append(mi(np.array(texto_v[i]),np.array(hipotesis_v[j]), base=2, alpha=0))
        lista_l.append(lista)
...
for i in range(len(textos)):
    ...
    sumas.append(ma.mean().sum())# cambie a minimo antes era suma
    if len(ma.columns)==0:
        max_info.append(0)
    else:
    ...
    mearts.append(m_earth.min().sum())#suma de sumas lo cambie a max
    mutinf.append(m_mi.max().sum())#cambie de suma a max
    distancias.append(m_distancia.max().sum()) #cambie de maximas a sumas
