...
def getWDM(d1,d2):
    ...
    #W_ = [nlp.wv[w] for w in vect.get_feature_names_out()]
    W_=[]
...
for i in range(len(textos)):
    ...
    sumas.append(ma.sum().sum())
    if len(ma.columns)==0:
        max_info.append(0)
    else:
        max_info.append(ma.sum().sum())#/(ma.shape[1]))) modificación1 ademas de max a sum
    entropias.append(entropia(ma.round(2).values.flatten())) # antes era 1
    m_distancia = obtener_distancia(t_vectors,h_vectors,t_clean,h_clean,b_col,b_index)
    
    m_earth=m_earth*m_distancia
    mearts.append(m_earth.sum().sum())
    mutinf.append(m_mi.sum().sum())
    distancias.append(m_distancia.max().sum()) 
    ...
    for t__1 in ma.index:
        st=st+" "+str(str(t__1).split("{")[0])
    for t__2 in ma.columns:
        sh=sh+" "+str(str(t__2).split("{")[0])
    doc1 = nlp(st)
    doc2 = nlp(sh)
...
temp1 =np.array([sumas,distancias,entropias,mutinf,mearts,max_info,list_antonimos,lista_related,similitud_faltantes,bien_me,clases])
df_resultados = pd.DataFrame(temp1.T,columns=["suma","distancias","entropias","mutual_info","m_earth","max_info_p","antonimos","relaciones","sim_faltantes","m_ea","CLASS"])
